...
class DaoFactory_Pro_Meta_Box {
	...
	public function add_metabox() {
		add_meta_box(
			'daofactory_pro_meta',
			esc_html__( 'Dao Factory Details', 'daofactory_pro' ),
	...
	public function render_metabox( $post ) {
		...
		wp_nonce_field( 'daofactory_pro_meta_action', 'daofactory_pro_meta_nonce' );

    $daoinfo = daofactory_pro_get_data($post->ID);
    $daoinfo_at_homepage = get_option( 'daofactory_pro_id_at_homepage', 'false');
    
		// Form fields.
    ?>
    <input type="hidden" id="daofactory_pro_post_id" value="<?php echo $post->ID?>" />
    <table class="form-table">
      <tr>
        <th><label><?php echo esc_html__( 'As home page', 'daofactory_pro' ); ?></label></th>
        <td>
          <input type="checkbox" name="dao_at_homepage" id="dao_at_homepage" <?php echo ($daoinfo_at_homepage == $post->ID) ? 'checked' : ''?>/>
          <label for="dao_at_homepage"><?php echo esc_html__( 'Show this DAO at home page', 'daofactory_pro' ); ?></label>
        </td>
      </tr>
      <tr>
        ...
        <td>
          ...
          <label for="dao_hide_footer_header"><?php echo esc_html__( 'Hide WP footer and header', 'daofactory_pro' ); ?></label>
        </td>
      </tr>
      <tr>
        <th><label><?php echo esc_html__( 'Hide service link', 'daofactory_pro' ); ?></label></th>
        <td>
          <input type="checkbox" name="dao_hide_service_link" id="dao_hide_service_link" <?php echo ($daoinfo['hide_service_link'] === 'true') ? 'checked' : ''?>/>
          <label for="dao_hide_service_link"><?php echo esc_html__( 'Hide service link on DAO page', 'daofactory_pro' ); ?></label>
        </td>
      </tr>
      <tr>
        ...
        <td>
          <select name="dao_theme" id="dao_theme" value="<?php echo $daoinfo['theme']?>">
            <option value="light" <?php echo ('light' === $daoinfo['theme']) ? 'selected' : ''?>><?php echo esc_html__('Light', 'daofactory_pro'); ?></option>
            <option value="dark" <?php echo ('dark' === $daoinfo['theme']) ? 'selected' : ''?>><?php echo esc_html__('Dark', 'daofactory_pro'); ?></option>
          </select>
        </td>
      </tr>
      <tr>
        <th><label><?php echo esc_html__( 'Blockchain', 'daofactory_pro' ); ?></label></th>
        <td>
          <select name="dao_blockchain" id="dao_blockchain" value="<?php echo $daoinfo['blockchain']?>">
            <?php
            foreach ( daofactory_pro_blockchains() as $key => $value) {
              ?>
      ...
      <tr>
        <th><label><?php echo esc_html__( 'Dao token Address', 'daofactory_pro' ); ?></label></th>
        <td>
          <div class="daofactory-form-inline">
            ...
            <a class="button button-secondary" id="daofactory_fetchtoken_button">
              <?php echo esc_html__( 'Fetch', 'daofactory_pro' ) ?>
            </a>
          </div>
          <p class="description">
          <?php
            echo sprintf(
              esc_html__(
                'ERC20 address of token&#039;s contract which users will be used. Press Fetch for get info about token. Free test tokens %s.', 'daofactory_pro'
              ),
              '<a href="https://github.com/bokkypoobah/WeenusTokenFaucet" target="_blank">https://github.com/bokkypoobah/WeenusTokenFaucet</a>'
            );
          ?>
          </p>
        </td>
      ...
      <tbody id="dao_token_info" <?php if (!$daoinfo['token_symbol']) echo ' style="display: none" '; ?>>
        <tr>
          <th><label><?php echo esc_html__( 'Token name', ' daofactory_pro' ); ?></label></th>
          <td>
        ...
        <tr>
          <th><label><?php echo esc_html__( 'Token symbol', ' daofactory_pro' ); ?></label></th>
          <td>
        ...
        <tr>
          <th><label><?php echo esc_html__( 'Token decimals', ' daofactory_pro' ); ?></label></th>
          <td>
      ...
      <tr>
        <th><label><?php echo esc_html__( 'Required tokens amount to Publish', 'daofactory_pro' ); ?></label></th>
        <td>
          <div class="daofactory-form-inline">
	...
	public function save_metabox( $post_id, $post ) {
		$nonce_name   = isset( $_POST['daofactory_pro_meta_nonce'] ) ? $_POST['daofactory_pro_meta_nonce'] : '';
		$nonce_action = 'daofactory_pro_meta_action';
    ...
    // DAO at home page
    $current_dao_at_homepage = get_option( 'daofactory_pro_id_at_homepage', 'false');
    ...
    if (!$set_this_at_homepage and ($current_dao_at_homepage !== 'false') and (intval($current_dao_at_homepage) == $post_id)) {
      delete_option('daofactory_pro_id_at_homepage');
    } else if ($set_this_at_homepage) {
      update_option('daofactory_pro_id_at_homepage', $post_id);
    }
	}

}

new DaoFactory_Pro_Meta_Box;

/**
 * Shortcode
 */

function daofactory_pro_post_submitbox( $post ) {
	if ( 'daofactory_pro' === $post->post_type ) {
		?>
		<div class="misc-pub-section">
			<p class="description"><strong><?php esc_html_e( 'Shortcode', 'daofactory_pro' ); ?><strong></p>
			<input type="text" class="large-text daofactory-schortcode-copy" value='[daofactory_pro id="<?php echo esc_attr( $post->ID ); ?>"]' readonly>
			<div class="copy-to-clipboard-container">
				<button type="button" class="button button-small copy-farm-shortcode" data-clipboard-target=".daofactory-schortcode-copy"><?php esc_html_e( 'Copy Shortcode to clipboard', 'daofactory_pro' ); ?></button>
				<span class="success hidden" aria-hidden="true"><?php esc_html_e( 'Copied!', 'daofactory_pro' ); ?></span>
			</div>
		</div>
		<?php
	}
}
add_action( 'post_submitbox_misc_actions', 'daofactory_pro_post_submitbox' );
